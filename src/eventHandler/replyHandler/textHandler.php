	...
	class textHandler implements eventHandler 
	{
		...
		public function handle () {
			...
			switch ($this->text) {
				...
				case 'DB':
					$q = $this->linedb->query("SELECT * FROM `member` WHERE id = '2'")->find();
					$this->textReply($q->name);
					break;

				case 'Multiple':
					$this->multiTextReply("S","U","T");
					break;
